...
pub async fn add_ch_sender(operation_id: String, ch: UnboundedSender<GatheredMessage>) {
    OPENED_CH.lock().await.insert(operation_id, ch);
}
...
pub async fn get_opened_ch_sender(operation_id: &str) -> Option<UnboundedSender<GatheredMessage>> {
    OPENED_CH.lock().await.get(operation_id).cloned()
}
