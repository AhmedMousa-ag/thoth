use crate::grpc::grpc_server::{
    MathOperations,
    mathop::{
        ListAverageOperationRequest, ListMaxRequest, ListMinRequest, OrderListRequest,
        math_ops_server::MathOps,
    },
};
...
async fn test_max_list() {
    ...
    let request = tonic::Request::new(ListMaxRequest {
        operation_id: format!("test_max_list_{}", uuid::Uuid::new_v4()),
        x: list_to_max.clone(),
    });
...
async fn test_min_list() {
    ...
    let request = tonic::Request::new(ListMinRequest {
        operation_id: format!("test_min_list_{}", uuid::Uuid::new_v4()),
        x: list_to_min.clone(),
    });
